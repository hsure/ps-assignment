...
public class DriverShipmentsApplication implements CommandLineRunner {
  ...
  private static final Logger logger = LoggerFactory.getLogger(
    DriverShipmentsApplication.class
  );
  ...
  public void run(String... args) throws Exception {
    ...
    if (args.length == 2) {
      ...
      try {
        List<Destination> destinations = destinationLoader.loadDestinations(
          destinationsFile
        );
        ...
        for (Map.Entry<Destination, Driver> entry : driverAssigner
          .mapDrivers(destinations, drivers)
          .entrySet()) {
      ...
      logger.info(
        "java -jar target/DriverShipments-0.0.1-SNAPSHOT.jar <destinations-file> <drivers-file> "
      );
    }
  }
}
